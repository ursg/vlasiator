...
void ipShockScenario::afterPush(int step, double time, ParticleContainer& particles, Field& E, Field& B, Field& V) {
   ...
   for (unsigned int i = 0; i < particles.size(); i++) {
      ...
      if (particles[i].x[0] < ParticleParameters::ipshock_transmit) {
         ...
         fprintf(traFile, "%lf %lf %lf %lf %lf %lf %lf %lf %lf\n", time, particles[i].x[0], particles[i].x[1], particles[i].x[2], particles[i].v[0], particles[i].v[1], particles[i].v[2],
                 .5 * particles[i].m * dot_product(particles[i].v, particles[i].v) / PhysicalConstantsSI::e, dot_product(normalize_vector(particles[i].v), normalize_vector(B(particles[i].x))));
