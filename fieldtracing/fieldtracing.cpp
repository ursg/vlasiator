...
// Trace node coordinates outwards until a non-sysboundary cell is encountered or the local fsgrid domain has been left.
            ...
            for (uint n = 0; n < nodes.size(); n++) {
               ...
               while (true) {
                  ...
                  stepFieldLine(x, v, nodeTracingStepSize[n], fieldTracingParameters.min_tracer_dx_full_box, technicalGrid.DX / 2, fieldTracingParameters.tracingMethod, tracingFullField,
                                (no.x[2] < 0));
   ...
   std::array<std::pair<int, Real>, 3> calculateIonosphereVlasovGridCoupling(std::array<Real, 3> x, std::vector<SBC::SphericalTriGrid::Node>& nodes, creal couplingRadius) {
      ...
      while (x[0] * x[0] + x[1] * x[1] + x[2] * x[2] > SBC::Ionosphere::innerRadius * SBC::Ionosphere::innerRadius) {
         ...
         // Make one step along the fieldline
         stepFieldLine(x, v, stepSize, fieldTracingParameters.min_tracer_dx_ionospere_coupling, fieldTracingParameters.max_tracer_dx_ionospere_coupling, fieldTracingParameters.tracingMethod,
...
// Trace node coordinates outwards until a non-sysboundary cell is encountered or the local fsgrid domain has been left.
            ...
            for (uint n = 0; n < nodes.size(); n++) {
               ...
               while (true) {
                  ...
                  stepFieldLine(x, v, nodeTracingStepSize[n], (TReal)fieldTracingParameters.min_tracer_dx_full_box, (TReal)technicalGrid.DX / 2, fieldTracingParameters.tracingMethod, tracingFullField,
                                (no.x[2] < 0));
   ...
   void stepCellAcrossTaskDomain(cint n, FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, TracingFieldFunction<TReal>& tracingFullField,
      ...
      while (true) {
         ...
         if (x.at(0) * x.at(0) + x.at(1) * x.at(1) + x.at(2) * x.at(2) < fieldTracingParameters.innerBoundaryRadius * fieldTracingParameters.innerBoundaryRadius) {
            ...
            // Take a step back and find the innerRadius crossing point
            stepFieldLine(x, v, cellTracingStepSize[n], (TReal)fieldTracingParameters.min_tracer_dx_full_box, (TReal)technicalGrid.DX / 2, fieldTracingParameters.tracingMethod, tracingFullField,
...
// Trace node coordinates forward and backwards until a non-sysboundary cell is encountered or the local fsgrid domain has been left.
            ...
            for (int n = 0; n < globalDccrgSize; n++) {
               if (cellFWConnection[n] % TracingLineEndType::N_TYPES == TracingLineEndType::UNPROCESSED) {
                  stepCellAcrossTaskDomain(n, technicalGrid, tracingFullField, cellInitialCoordinates, cellCurvatureRadius, cellFWTracingCoordinates, cellFWTracingStepSize, cellFWRunningDistance,
                                           cellMaxExtension, cellFWConnection, warnMaxDistanceExceeded, maxTracingDistance, Direction::FORWARD);
               }
               if (cellBWConnection[n] % TracingLineEndType::N_TYPES == TracingLineEndType::UNPROCESSED) {
                  stepCellAcrossTaskDomain(n, technicalGrid, tracingFullField, cellInitialCoordinates, cellCurvatureRadius, cellBWTracingCoordinates, cellBWTracingStepSize, cellBWRunningDistance,
                                           cellMaxExtension, cellBWConnection, warnMaxDistanceExceeded, maxTracingDistance, Direction::BACKWARD);
               }
            } // for
