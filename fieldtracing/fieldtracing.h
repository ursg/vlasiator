...
template <class T> std::array<FsGridTools::FsIndex_t, 3> getLocalFsGridCellIndexWithGhostsForCoord(T& grid, const std::array<Real, 3>& x) {
   ...
   std::array<FsGridTools::FsIndex_t, 3> retval = {(FsGridTools::FsIndex_t)globalInd[0] - localStart[0], (FsGridTools::FsIndex_t)globalInd[1] - localStart[1],
                                                   (FsGridTools::FsIndex_t)globalInd[2] - localStart[2]};
   return retval;
}
   ...
   bool traceFullFieldFunction(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
      ...
      if (fsgridCell[0] > localSize[0] || fsgridCell[1] > localSize[1] || fsgridCell[2] > localSize[2] || fsgridCell[0] < -1 || fsgridCell[1] < -1 || fsgridCell[2] < -1) {
         ...
         if (technicalGrid.get(fsgridCell[0], fsgridCell[1], fsgridCell[2])->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY) {
            const std::array<Real, 3> perB = interpolatePerturbedB(perBGrid, dPerBGrid, technicalGrid, fieldTracingParameters.reconstructionCoefficientsCache, fsgridCell[0], fsgridCell[1],
                                                                   fsgridCell[2], {(Real)r[0], (Real)r[1], (Real)r[2]});
            b[0] += perB[0];
            b[1] += perB[1];
            b[2] += perB[2];
         }
