...
void compute_intersections_2nd(const vmesh::VelocityMesh* vmesh, const Transform<Real, 3, Affine>& bwd_transform, const Transform<Real, 3, Affine>& fwd_transform, uint dimension, Real& intersection,
   ...
   if (dimension == 0) { // Prepare intersections for mapping along X second (mapping order Z-X-Y)
      ...
      const Eigen::Matrix<Real, 3, 1> line_point = bwd_transform * Eigen::Matrix<Real, 3, 1>(vmesh->getMeshMinLimits()[0], 0.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
   ...
   if (dimension == 1) { // Prepare intersections for mapping along Y second (mapping order X-Y-Z)
      ...
      const Eigen::Matrix<Real, 3, 1> line_point = bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], vmesh->getMeshMinLimits()[1],
                                                                                             0.5 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
   ...
   if (dimension == 2) { // Prepare intersections for mapping along Z second (mapping order Y-Z-X)
      ...
      const Eigen::Matrix<Real, 3, 1> line_point = bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0],
                                                                                             0.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1], vmesh->getMeshMinLimits()[2]);
...
void compute_intersections_3rd(const vmesh::VelocityMesh* vmesh, const Transform<Real, 3, Affine>& bwd_transform, const Transform<Real, 3, Affine>& fwd_transform, uint dimension, Real& intersection,
   ...
   if (dimension == 0) { // Prepare intersections for mapping along X third (mapping order Y-Z-X)
      const Eigen::Matrix<Real, 3, 1> point_0_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.0 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], 0.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
                                                    0.5 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_1_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(1.0 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], 0.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
                                                    0.5 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_1_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.0 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], 1.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
                                                    0.5 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
   ...
   if (dimension == 1) { // Prepare intersections for mapping along Y third (mapping order Z-X-Y)
      // This is the case presented in the Slice 3D article,
      // data along z has been moved to Lagrangian coordinates
      const Eigen::Matrix<Real, 3, 1> point_0_0_0 = bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], vmesh->getMeshMinLimits()[1],
                                                                                              0.5 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_1_0_0 = bwd_transform * Eigen::Matrix<Real, 3, 1>(1.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], vmesh->getMeshMinLimits()[1],
                                                                                              0.5 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_1_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], 1.0 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
                                                    0.5 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_0_1 = bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], vmesh->getMeshMinLimits()[1],
                                                                                              1.5 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      intersection = point_0_0_0[dimension];
      intersection_di = point_1_0_0[dimension] - point_0_0_0[dimension];
      intersection_dj = point_0_1_0[dimension] - point_0_0_0[dimension];
   ...
   if (dimension == 2) { // Prepare intersections for mapping along Z third (mapping order X-Y-Z)
      const Eigen::Matrix<Real, 3, 1> point_0_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], 0.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
                                                    0.0 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_1_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(1.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], 0.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
                                                    0.0 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_1_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], 1.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
                                                    0.0 * vmesh->getCellSize()[2] + vmesh->getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_0_1 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh->getCellSize()[0] + vmesh->getMeshMinLimits()[0], 0.5 * vmesh->getCellSize()[1] + vmesh->getMeshMinLimits()[1],
