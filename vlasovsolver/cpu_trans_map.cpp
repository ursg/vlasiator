   ...
   for (int i = VLASOV_STENCIL_WIDTH; i < (int)lengthOfPencil - VLASOV_STENCIL_WIDTH; i++) {
      ...
      for (uint k = 0; k < WID; ++k) {
         ...
         for (uint planeVector = 0; planeVector < VEC_PER_PLANE; planeVector++) {
            ...
            compute_ppm_coeff_nonuniform(dz + i - VLASOV_STENCIL_WIDTH, values + i_trans_ps_blockv_pencil(planeVector, k, i, lengthOfPencil) - VLASOV_STENCIL_WIDTH, h4, VLASOV_STENCIL_WIDTH, a,
                                         threshold);
