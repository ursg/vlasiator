...
__host__ bool gpu_acc_map_1d(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, vector<CellID>& launchCells, const uint popID, const uint dimension,
   ...
   */
   ...
   prefill_probe_kernel<<<grid_prefill_probe, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_vmeshes,
                                                                                                    ...
                                                                                                    flatExtent, Dacc, Dother, invalidLocalID,
                                                                                                    // Pass vectors for clearing
                                                                                                    dev_lists_with_replace_new, dev_lists_delete, dev_lists_to_replace, dev_lists_with_replace_old,
   ...
   fill_probe_ordered<<<grid_fill_ord, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_vmeshes,
                                                                                             dev_blockDataOrdered, // recast to vmesh::LocalID *probeCube
                                                                                             flatExtent, gpu_block_indices_to_probe, cumulativeOffset);
   CHK_ERR(gpuPeekAtLastError());
   CHK_ERR(gpuStreamSynchronize(baseStream));
   fillTimer.stop();
   ...
   flatten_probe_cube<<<grid_cube, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_blockDataOrdered, // recast to vmesh::LocalID *probeCube, *probeFlattened
                                                                                         Dacc, Dother, flatExtent, invalidLocalID);
   CHK_ERR(gpuPeekAtLastError());
   CHK_ERR(gpuStreamSynchronize(baseStream));
   flattenTimer.stop();
   ...
   */
   ...
   scan_probe<<<grid_scan, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_vmeshes,
                                                                                 ...
                                                                                 Dacc, Dother, flatExtent, dev_nColumns, dev_nColumnSets, dev_resizeSuccess, dev_columnOffsetData, cumulativeOffset);
   CHK_ERR(gpuPeekAtLastError());
   ...
   build_column_offsets<<<grid_cube, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_vmeshes,
                                                                                           ...
                                                                                           D0, D1, D2, dimension, flatExtent, invalidLocalID, dev_columnOffsetData,
   ...
   reorder_blocks_by_dimension_kernel<<<grid_reorder, block_reorder, 0, baseStream>>>(dev_VBCs, dev_blockDataOrdered, gpu_cell_indices_to_id,
                                                                                      dev_vbwcl_vec, // dev_velocity_block_with_content_list, // use as LIDlist
                                                                                      dev_columnOffsetData, dev_nColumns, cumulativeOffset);
   CHK_ERR(gpuPeekAtLastError());
   CHK_ERR(gpuStreamSynchronize(baseStream));
   reorderTimer.stop();
   ...
   evaluate_column_extents_kernel<<<grid_column_extents, GPUTHREADS, 0, baseStream>>>(dimension, dev_vmeshes, dev_columnOffsetData, dev_lists_with_replace_new, dev_allMaps, gpu_block_indices_to_id,
                                                                                      dev_intersections, Parameters::bailout_velocity_space_wall_margin, max_v_length, v_min, dv,
   ...
   if (needSecondLaunchColumnExtents) {
      ...
      CHK_ERR(gpuMemcpyAsync(dev_lists_with_replace_new + cumulativeOffset, host_lists_with_replace_new + cumulativeOffset, nLaunchCells * sizeof(split::SplitVector<vmesh::GlobalID>*),
   ...
   */
   ...
   acceleration_kernel<<<grid_acc, block_acc, 0, baseStream>>>(dev_vmeshes,          // indexing: cellOffset
                                                               ...
                                                               gpu_cell_indices_to_id, gpu_block_indices_to_id,
                                                               ...
                                                               invalidLocalID, cumulativeOffset);
   CHK_ERR(gpuPeekAtLastError());
   CHK_ERR(gpuStreamSynchronize(baseStream));
   accTimer.stop();
