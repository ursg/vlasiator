...
bool trans_map_1d_amr(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID>& localPropagatedCells, const vector<CellID>& remoteTargetCells,
   ...
   translation_kernel<<<grid, block, 0, bgStream>>>(dimension, dt, pencilLengths, pencilStarts,
                                                    ...
                                                    gpuMemoryManager.getPointer<uint>(DimensionPencils[dimension].dev_binSize), numberOfBins);
   CHK_ERR(gpuPeekAtLastError());
   CHK_ERR(gpuStreamSynchronize(bgStream));
   mappingTimer.stop();
