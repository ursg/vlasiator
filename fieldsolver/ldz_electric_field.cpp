...
void calculateEdgeElectricFieldX(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid,
   ...
   calculateWaveSpeedYZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k, i + 1, j - 1, k, By_S, Bz_E, dBydx_S, dBydz_S, dBzdx_E, dBzdy_E, PLUS, MINUS, minRhom, maxRhom, vA, vS,
                        vW);
   c_y = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_z = c_y;
   ...
   calculateWaveSpeedYZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k - 1, i + 1, j, k - 1, By_N, Bz_W, dBydx_N, dBydz_N, dBzdx_W, dBzdy_W, MINUS, PLUS, minRhom, maxRhom, vA, vS,
                        vW);
   c_y = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_z = c_y;
   ay_neg = max(ay_neg, -Vy0 + c_y);
   ...
   calculateWaveSpeedYZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k - 1, i + 1, j - 1, k - 1, By_N, Bz_E, dBydx_N, dBydz_N, dBzdx_E, dBzdy_E, PLUS, PLUS, minRhom, maxRhom, vA,
                        vS, vW);
   c_y = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_z = c_y;
   ay_neg = max(ay_neg, -Vy0 + c_y);
...
void calculateEdgeElectricFieldY(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid,
   ...
   calculateWaveSpeedXZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k - 1, i, j + 1, k - 1, Bx_E, Bz_S, dBxdy_E, dBxdz_E, dBzdx_S, dBzdy_S, MINUS, PLUS, minRhom, maxRhom, vA, vS,
                        vW);
   c_z = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_x = c_z;
   az_neg = max(az_neg, -Vz0 + c_z);
   ...
   calculateWaveSpeedXZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k, i - 1, j + 1, k, Bx_W, Bz_N, dBxdy_W, dBxdz_W, dBzdx_N, dBzdy_N, PLUS, MINUS, minRhom, maxRhom, vA, vS,
                        vW);
   c_z = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_x = c_z;
   az_neg = max(az_neg, -Vz0 + c_z);
   ...
   calculateWaveSpeedXZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k - 1, i - 1, j + 1, k - 1, Bx_E, Bz_N, dBxdy_E, dBxdz_E, dBzdx_N, dBzdy_N, PLUS, PLUS, minRhom, maxRhom, vA,
                        vS, vW);
   c_z = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_x = c_z;
   az_neg = max(az_neg, -Vz0 + c_z);
...
void calculateEdgeElectricFieldZ(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid,
   ...
   calculateWaveSpeedXY(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k, i - 1, j, k + 1, Bx_S, By_E, dBxdy_S, dBxdz_S, dBydx_E, dBydz_E, PLUS, MINUS, minRhom, maxRhom, vA, vS,
                        vW);
   c_x = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_y = c_x;
   ax_neg = max(ax_neg, -Vx0 + c_x);
   ...
   calculateWaveSpeedXY(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k, i, j - 1, k + 1, Bx_N, By_W, dBxdy_N, dBxdz_N, dBydx_W, dBydz_W, MINUS, PLUS, minRhom, maxRhom, vA, vS,
                        vW);
   c_x = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_y = c_x;
   ax_neg = max(ax_neg, -Vx0 + c_x);
   ...
   calculateWaveSpeedXY(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j - 1, k, i - 1, j - 1, k + 1, Bx_N, By_E, dBxdy_N, dBxdz_N, dBydx_E, dBydz_E, PLUS, PLUS, minRhom, maxRhom, vA,
                        vS, vW);
   c_x = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_y = c_x;
   ax_neg = max(ax_neg, -Vx0 + c_x);
