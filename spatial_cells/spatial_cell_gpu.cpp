   ...
   */
   template <typename fileReal> void SpatialCell::add_velocity_blocks(const uint popID, const std::vector<vmesh::GlobalID>& blocks, fileReal* initBuffer) {
      ...
      if (nBlocks > 0) {
         ...
         CHK_ERR(gpuStreamSynchronize(stream));
         spatial_cell::add_blocks_from_buffer_kernel<<<nBlocks, block, 0, stream>>>(populations[popID].dev_vmesh, populations[popID].dev_blockContainer, startLID, gpuInitBlocks, gpuInitBuffer,
                                                                                    nBlocks);
         CHK_ERR(gpuPeekAtLastError());
      }
      CHK_ERR(gpuStreamSynchronize(stream));
