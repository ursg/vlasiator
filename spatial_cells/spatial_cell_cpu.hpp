...
namespace spatial_cell {
   ...
   void SpatialCell::adjust_velocity_blocks(const uint popID, bool doDeleteEmptyBlocks) {
      ...
      if (velocity_block_with_content_list_size > 0) {
         ...
         update_velocity_halo_kernel<<<velocity_block_with_content_list_size, 26 * 32, 0, stream>>>(dev_vmesh, velocity_block_with_content_list_size, _withContentData,
                                                                                                    dev_velocity_block_with_content_map, dev_velocity_block_with_no_content_map);
      ...
      */
      ...
      if (doDeleteEmptyBlocks) {
         ...
         auto rule_delete_move = [emptybucket, tombstone, vbwncm, d_list_add, dev_vmesh, invalidGID,
                                  invalidLID] __device__(const Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>& kval) -> bool {
   ...
   **/
   vmesh::LocalID SpatialCell::adjust_velocity_blocks_caller(const uint popID) {
      ...
      resize_vbc_kernel_pre<<<1, 1, 0, stream>>>(populations[popID].dev_vmesh, populations[popID].dev_blockContainer, dev_list_with_replace_new, dev_list_delete, dev_list_to_replace,
      ...
      // CHK_ERR( gpuStreamSynchronize(stream) );
      update_velocity_blocks_kernel<<<launchBlocks, vlasiBlocksPerWorkUnit * WID3, 0, stream>>>(populations[popID].dev_vmesh, populations[popID].dev_blockContainer, dev_list_with_replace_new,
                                                                                                dev_list_delete, dev_list_to_replace, dev_list_with_replace_old, nBlocksBeforeAdjust, nBlocksToChange,
                                                                                                nBlocksAfterAdjust,
