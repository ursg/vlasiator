...
#pragma omp critical
      ...
      batch_update_velocity_block_content_lists_kernel<<<grid2, WID3, 0, baseStream>>>(dev_vmeshes, dev_VBCs, dev_allMaps, dev_minValues,
      ...
      auto rule = [] __device__(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* map, const Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>& kval, const vmesh::LocalID threshold,
                                const vmesh::LocalID invalidLID, const vmesh::GlobalID invalidGID) -> bool {
...
#pragma omp critical
      ...
      CHK_ERR(gpuMemcpyAsync(dev_lists_with_replace_old, host_lists_with_replace_old, nCells * sizeof(split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>*),
      ...
      if (largestContentList > 0) {
         ...
         batch_update_velocity_halo_kernel<<<grid_vel_halo, 26 * 32, 0, priorityStream>>>(dev_vmeshes, dev_vbwcl_vec,
         ...
         batch_update_velocity_halo_kernel<<<grid_vel_halo, block_vel_halo, 0, priorityStream>>>(dev_vmeshes, dev_vbwcl_vec,
                                                                                                 dev_allMaps, // Needs both content and no content maps
...
// largestContentListNeighbors accounts for remote (ghost neighbor) content list sizes as well
      ...
      */
      ...
      batch_adjust_blocks_caller(mpiGrid, cellsToAdjust,
                                 0, // no offset
                                 largestBlocksToChange, largestBlocksBeforeOrAfter, popID);
      ...
      clean_tombstones_launcher<decltype(rule_overflown)>(dev_vmeshes,                // velocity meshes which include the hash maps to clean
                                                          ...
                                                          rule_overflown, nCells, baseStream);
...
#pragma omp for schedule(dynamic)
   ...
   void batch_adjust_blocks_caller(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID>& cellsToAdjust, const uint cellOffset,
      ...
      batch_resize_vbc_kernel_pre<<<nCells, 1, 0, baseStream>>>(dev_vmeshes + cellOffset, dev_VBCs + cellOffset, dev_lists_with_replace_new + cellOffset, dev_lists_delete + cellOffset,
                                                                dev_lists_to_replace + cellOffset, dev_lists_with_replace_old + cellOffset, dev_nBefore + cellOffset, dev_nAfter + cellOffset,
                                                                dev_nBlocksToChange + cellOffset, dev_resizeSuccess + cellOffset,
      ...
      if (largestBlocksToChange > 0) {
         ...
         batch_update_velocity_blocks_kernel<<<grid_addremove, WID3, 0, baseStream>>>(dev_vmeshes + cellOffset, dev_VBCs + cellOffset, dev_lists_with_replace_new + cellOffset,
                                                                                      dev_lists_delete + cellOffset, dev_lists_to_replace + cellOffset, dev_lists_with_replace_old + cellOffset,
                                                                                      dev_nBefore + cellOffset, dev_nAfter + cellOffset, dev_nBlocksToChange + cellOffset, dev_massLoss + cellOffset);
         CHK_ERR(gpuPeekAtLastError());
   ...
   void extract_to_replace_caller(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** input_maps, split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>** output_vecs,
      ...
      auto rule_to_replace = [] __device__(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* map, const Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>& kval,
                                           const vmesh::LocalID threshold, const vmesh::LocalID invalidLID, const vmesh::GlobalID invalidGID) -> bool {
         const vmesh::GlobalID emptybucket = map->get_emptybucket();
         const vmesh::GlobalID tombstone = map->get_tombstone();
      ...
      // Find Blocks (GID,LID) to be replaced with new ones
      extract_GIDs_kernel_launcher<decltype(rule_to_replace), Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>, false>(input_maps, output_vecs, output_sizes, rule_to_replace, rule_meshes,
                                                                                                                             rule_maps, rule_vectors, nCells, stream);
   }
   ...
   void extract_to_delete_or_move_caller(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** input_maps, split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>** output_vecs,
                                         vmesh::LocalID* output_sizes, vmesh::VelocityMesh** rule_meshes, Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** rule_maps,
                                         split::SplitVector<vmesh::GlobalID>** rule_vectors, const uint nCells, gpuStream_t stream) {
      auto rule_delete_move = [] __device__(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* map, const Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>& kval,
                                            const vmesh::LocalID threshold, const vmesh::LocalID invalidLID, const vmesh::GlobalID invalidGID) -> bool {
         const vmesh::GlobalID emptybucket = map->get_emptybucket();
         const vmesh::GlobalID tombstone = map->get_tombstone();
         return kval.first != emptybucket && kval.first != tombstone && kval.first != invalidGID && kval.second >= threshold && kval.second != invalidLID;
      };
      extract_GIDs_kernel_launcher<decltype(rule_delete_move), Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>, false>(input_maps, output_vecs, output_sizes, rule_delete_move, rule_meshes,
                                                                                                                              rule_maps, rule_vectors, nCells, stream);
   }
   ...
   void extract_to_add_caller(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** input_maps, split::SplitVector<vmesh::GlobalID>** output_vecs, vmesh::LocalID* output_sizes,
      ...
      auto rule_add = [] __device__(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* map, const Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>& kval, const vmesh::LocalID threshold,
                                    const vmesh::LocalID invalidLID, const vmesh::GlobalID invalidGID) -> bool {
         // This rule does not use the threshold value
         const vmesh::GlobalID emptybucket = map->get_emptybucket();
         const vmesh::GlobalID tombstone = map->get_tombstone();
