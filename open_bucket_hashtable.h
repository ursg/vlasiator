...
namespace OBH{
   ...
   struct HashFunction{
      ...
      //Hash a byte using fnv_1a
      uint32_t fnv_1a(unsigned char byte, uint32_t h) const {
         return h = (h ^ byte) * 16777619ul;
      }

      //Hash a chunk of memory using fnv_1a
      uint32_t fnv_1a(const void* chunk, size_t bytes, uint32_t h) const{
         assert(chunk);
      ...
      //Hash a uint32_t using multiplicative fibonacci hashing 
      uint32_t fibonacci(uint32_t in, int sizePower) const {
         in ^= in >> (32 - sizePower);
      ...
      template <typename T> uint32_t hash(T in,int powerSize)const{ 
         throw std::runtime_error("OpenBucketHashtable: No specialization for this datatype");
         return 0;
      }
   ...
   template <> inline uint32_t HashFunction::hash<unsigned short>(unsigned short in, int powerSize) const {
      const unsigned char* ptr = (const unsigned char*)&in;
      uint32_t h = 2166136261ul;
      h=fnv_1a(*ptr++,h);
      return h=fnv_1a(*ptr++,h);
   }

   // Specialization for uint32_t, here we use Fibonacci
   template <> inline uint32_t HashFunction::hash<uint32_t>(uint32_t in, int powerSize) const {
      return fibonacci(in, powerSize);
   }
   
   //Specialization for uint64_t
   template <> inline uint32_t HashFunction::hash<uint64_t>(uint64_t in, int powerSize) const {
      const unsigned char* ptr = (const unsigned char*)&in;
      uint32_t h = 2166136261ul;
      h=fnv_1a(*ptr++,h);
      h=fnv_1a(*ptr++,h);
   ...
   template <> inline uint32_t HashFunction::hash<float>(float in, int powerSize) const {
      uint32_t h = 2166136261ul;
      return fnv_1a(&in, sizeof(in), h);
   }
   // Specialization for double
   template <> inline uint32_t HashFunction::hash<double>(double in, int powerSize) const {
      uint32_t h = 2166136261ul;
      return fnv_1a(&in, sizeof(in), h);
   }

   // Open bucket power-of-two sized hash table
   ...
   private:
      ...
      uint32_t hash(GID in) const {
         return hashFun.hash<GID>(in,sizePower);
      }
